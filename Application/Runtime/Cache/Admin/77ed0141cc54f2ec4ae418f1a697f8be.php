...
<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form class="form-horizontal tasi-form" method="get">
        <div class="form-group">
          <label class="col-sm-2 control-label">标题</label>
          <div class="col-sm-10">
            <input type="text" class="form-control" id="title" name="title" value="<?php echo ($vo["filename"]); ?>">
            <input type="hidden" class="form-control" id="fid" name="fid" value="<?php echo ($vo["id"]); ?>">
          </div>
        </div>
        <div class="form-group">
          <label class="col-sm-2 control-label">描述</label>
          <div class="col-sm-10">
            <input type="text" class="form-control" id="describe" name="describe" value="<?php echo ($vo["describe"]); ?>">
          ...
          <div class="row">
            <div id="crop-avatar" class="col-md-6">
              <div class="avatar-view" style="width:60px;height:60px">
                ...
                <img src="/Public/img/uploads/fileimg/<?php echo ($vo["img"]); ?>" alt="点击修改图片" style="width:70px;height:70px">
              </div>
            </div>
          </div>
      </div>
        ...
        <!-- 分类 -->
            <select class="form-control m-bot15" name="type" id="type">
              <?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i; if($vo['fid'] == $vos['id']){ $sel="selected='selected'"; }else{ $sel=""; } ?>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-10">
        <!-- 分类 -->
        <?php if($vo["status"] == 1): ?><select class="form-control m-bot15" name="status" id="status">
        ...
        <!-- 按钮组 -->
            <button type="button" class="btn btn-success" onclick="editarticle()">修改</button>
...
<div class="modal fade" id="avatar-modal" aria-hidden="true" aria-labelledby="avatar-modal-label" role="dialog" tabindex="-1">
  <div class="modal-dialog modal-lg"  style="width:1000px;">
    <div class="modal-content">
      <form class="avatar-form" action="<?php echo U('Article/editarticleimg',array('id'=>$id));?>" enctype="multipart/form-data" method="post">
...
<script type="text/javascript">
    function editarticle(){
       ...
       var fid=$('#type').val();
       var id=$('#fid').val();
       var ajaxarticleUrl='<?php echo U("Article/updatearticle");?>'; 
       if(filename==''){
          layer.msg('亲，请填写标题！');
          return false;
       }
    ...
    $.ajax({
      ...
      //提交的网址
        url: ajaxarticleUrl,
      //提交的数据
        data: {filename: filename,describe:describe,content:content,status:status,fid:fid,id:id},
      //返回数据的格式
        datatype: "json",
      //成功返回之后调用的函数
        success: function (data) {
