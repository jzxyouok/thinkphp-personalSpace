              ...
              <!--普通用户添加-->
                ...
                <?php if(is_array($date)): $i = 0; $__LIST__ = $date;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form role="form">
                    <div class="form-group">
                      <label for="exampleInputEmail1">头像
                      </label>
                      <!---->
                      <!--头像-->
                      <div class="ibox-content">
                        <div class="row">
                            <div id="crop-avatar" class="col-md-6">
                              <div class="avatar-view" style="width:160px;height:160px">
                              <input  id="img" name="img" type="hidden" value="<?php echo ($vo["img"]); ?>">
                              <input  id="aspectRatio" type="hidden" value="1">
                              <input  id="img_root_url" type="hidden" value="/Public/img/uploads/imgs">
                              <img src="/Public/img/uploads/imgs/<?php echo ($vo["img"]); ?>" alt="点击修改图片" style="width:160px;height:160px">
                              </div>
                            </div>
                        </div>
                      </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" class="form-control" id="imgid"  name="imgid" value="<?php echo ($vo["id"]); ?>">
                    </div>
                    <button type="button" class="btn btn-info" onClick="updateimg()">修改
                    </button>
...
<div class="modal fade" id="avatar-modal" aria-hidden="true" aria-labelledby="avatar-modal-label" role="dialog" tabindex="-1">
  <div class="modal-dialog modal-lg"  style="width:1000px;">
    <div class="modal-content">
      <form class="avatar-form" action="<?php echo U('Image/updateimg',array('id'=>$imgid));?>" enctype="multipart/form-data" method="post">
        <div class="modal-header">
          <button class="close" data-dismiss="modal" type="button">&times;</button>
          <h4 class="modal-title" id="avatar-modal-label">图片修改</h4>
        </div>
...
<!-- 密码修改 -->
  ...
  <script type="text/javascript">
    function updateimg(){
		var imgdescribe=$('#describe').val();
		var imgid=$('#imgid').val();
		var ajaximgUrl='<?php echo U("Image/editimg");?>'; 
    //<!--ajax提交-->
	    $.ajax({
      //提交数据的类型 POST GET
        type: "POST",
      //提交的网址
        url: ajaximgUrl,
      //提交的数据
        data: {imgdescribe:imgdescribe,imgid:imgid},
      //返回数据的格式
        datatype: "json",
      //成功返回之后调用的函数
        success: function (data) {
